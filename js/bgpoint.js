...
fn perlinNoise3(P: vec3f) -> f32 {
    ...
    fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
        ...
        let offset = vec3<f32>(time*5.,0,0)+vec3<f32>(rand33(vec3<f32>(global_id)))*.1;
        
        
        particleVelocities[index].x += perlinNoise3(particlePositions[index]*2.+offset)*.2;
        particleVelocities[index].y += perlinNoise3(particlePositions[index]*2.+10.-offset)*.2;
        particleVelocities[index] *= .95;
        particleVelocities[index] = clamp(particleVelocities[index], vec3<f32>(-1), vec3<f32>(1));

        // Update position
        particlePositions[index] += particleVelocities[index] * 0.01;

        // Keep particles within bounds [-1, 1]
        for (var i = 0; i < 3; i++) {
            if (particlePositions[index][i] < -1.0) {
